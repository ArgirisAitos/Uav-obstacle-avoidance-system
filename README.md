...
The live streaming service is automatically started when `obstacle.py` is executed.

The system receives the camera stream generated by `rpicam-vid`, processes each frame using the YOLO model, and serves the annotated video through a Flask web server.

Open:
``
http://RaspberryPi-IP:5000/video
``
